...
export default function Login() {
  const { login, error, setError, demo } = useAuth()
  ...
  function onSubmit(e) {
    e.preventDefault()
    if (login(email, password)) navigate(from, { replace: true })
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 px-4 dark:bg-zinc-950">
      <div className="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <p className="mt-2 text-sm text-zinc-500">
          Demo account: <span className="font-medium text-zinc-800 dark:text-zinc-200">{demo.email}</span> / {demo.password}
        </p>
        ...
        <p className="mt-6 text-center text-sm text-zinc-500">
          No account?{' '}
          <Link to="/signup" className="font-medium text-zinc-950 dark:text-white">
            Create one
          </Link>
        </p>
      </div>
